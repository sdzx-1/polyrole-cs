...
// 所有 N 个客户端统计收到的 chat 消息（SharedBoard 模式下发送者也经消息板
// 拉回自己的消息，期望 = N*msgs），并测量首条/末条广播延迟。
// 覆盖：SharedBoard O(1) 广播、Push 游标拉取、推送吞吐。
//
// 服务器建议配合 `--silent-join`（关闭加入/离开通知）：否则同时注册 N 人
// 会产生 O(N²) 的加入通知刷板（SharedBoard 无物理回收，1M 条容量）。
// batch_size < N 时按批注册（每批后等待 2s），用于旧架构（逐连接 inbox）
// 或内存受限场景；SharedBoard 架构下广播 = O(1) append，无需分批
// （实测 10000 人同时注册 + 广播 100% 送达）。
