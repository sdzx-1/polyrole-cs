...
其余 N-1 个客户端统计收到的 chat 消息（期望 (N-1)*msgs）与广播延迟：

```bash
# 100 连接不分批：广播机制验证
zig-out/bin/chat-loadtest 100 127.0.0.1 7788 10 10

# 1000 连接分批（10 批 × 100，批间 2s 消化加入通知）
zig-out/bin/chat-loadtest 1000 127.0.0.1 7788 30 10 100
```
...
|---|---|---|---|---|
| 100 | 10 条 × 99 接收方 | 100% | 4ms | 50ms |
| 1000（分批） | 10 条 × 999 | 100% | 95ms | 216ms |

**已知边界（10000 广播）**：同时注册 N 人会产生 **O(N²) 的加入通知风暴**
（10000 人时单连接累计收 ~9999 条加入通知）。服务器单线程 Push 推送消化
不及，触发**慢消费者 fail-fast 断开**（Room count 峰值远低于 N，广播送达率低）。
这是"万人同时加入"极端场景的系统保护行为；真实系统渐进加入（如每分钟
几十人）不会触发。要压 10000 在线时的广播，需要先建立在线连接（不触发
加入风暴）再广播——多进程压测形态或协议级"静默注册"开关，留作后续工作
（见 §3.5 扩展点 B）。
